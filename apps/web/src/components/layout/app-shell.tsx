 ...
 *
 * O entorno é `bg-shell`, e só ele: sem isso o desktop fica branco sobre
 * branco e a moldura de celular desaparece. O `main` continua `bg-background`
 * — `tests/shell.test.tsx` guarda que ele não ganha variante de largura.
 *
...
export function AppShell({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-[100dvh] justify-center bg-shell">
      <main className="relative flex h-[100dvh] w-full max-w-[393px] flex-col overflow-hidden bg-background">
        <div className="flex-1 overflow-y-auto pt-[max(1.5rem,env(safe-area-inset-top))]">
          {children}
        </div>
        <TabBar />
      </main>
